...
use {
    ...
    crate::service::{LOCDService, LOCDCommands},
...
pub fn main() -> Result<()> {
    ...
    let mut ftp0 = LOCDService::new(rs0, response_sid.clone());
    let mut ftp1 = LOCDService::new(rs1, request_sid.clone());
    ...
    ftp1_c2p_tx.send(LOCDCommands::RequestFileList(hbfi0.clone()))?;
    let files0 = ftp1_p2c_rx.recv();
    debug!("\t\t\t\t\tprotocol-to-client");
    if let LOCDCommands::ResponseFileList(Some(files)) = files0? {
        debug!("\t\t\t\t\tfiles 0: {:?}", files);
        for file_name in files {
            debug!("\t\t\t\t\tclient-to-protocol");
            ftp1_c2p_tx.send(LOCDCommands::RequestFile(hbfi0.clone(), file_name.clone()))?;
            if let LOCDCommands::ResponseFile(Some(actual_file)) = ftp1_p2c_rx.recv()? {
                debug!("\t\t\t\t\tprotocol-to-client");
